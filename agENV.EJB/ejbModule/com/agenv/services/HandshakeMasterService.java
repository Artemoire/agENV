...
public class HandshakeMasterService {
	...
	public void registerNode(Node node) {
		...
		for (Node myNode : envBean.getNodes()) {
		...
		ClientBuilder.newClient().target("http://"+node.getCenter().getAddress()+"/node").request().post(
				Entity.json(envBean.getNodes()));
		
		// Step 4: send running agents to new node
		ClientBuilder.newClient().target("http://"+node.getCenter().getAddress()+"/agents/running").request().post(
				Entity.json(envBean.getAgents()));		
	}
